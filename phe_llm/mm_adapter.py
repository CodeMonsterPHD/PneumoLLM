...
class RepAdapter_Router(nn.Module):
    ...
    def __init__(
        ...
        # self.conv_D = nn.Conv1d(hidden_dim, in_features, 3, padding=1, groups=groups, bias=True)
...
class RepconvAdapter(nn.Module):
    """
    Pytorch Implemention of RepAdapter for 1d tensor
    ...
    def __init__(
        ...
        super().__init__()
        # self.conv_clsA = nn.Conv1d(in_features, hidden_dim, 1, groups=1, bias=True)
        # self.conv_clsB = nn.Conv1d(hidden_dim, in_features, 1, groups=1, bias=True)
        self.conv_A=nn.Conv2d(in_features,hidden_dim,1,padding=0,bias=True)
        self.conv_A_2 = nn.Conv2d(hidden_dim, hidden_dim, 3, padding=1, bias=True)
        self.conv_B_1 = nn.Conv2d(hidden_dim, hidden_dim, 3, padding=1, bias=True)
        self.conv_B = nn.Conv2d(hidden_dim, in_features, 1,padding=0, bias=True)
        ...
        # nn.init.xavier_uniform_(self.conv_clsA.weight)
        # nn.init.zeros_(self.conv_clsA.bias)
        # nn.init.zeros_(self.conv_clsB.weight)
        # nn.init.zeros_(self.conv_clsB.bias)
        nn.init.xavier_uniform_(self.conv_A.weight)
        # nn.init.xavier_normal_(self.conv_A.weight)
        nn.init.zeros_(self.conv_A.bias)
        nn.init.zeros_(self.conv_B.weight)
        nn.init.zeros_(self.conv_B.bias)

    def forward(self, x,weights=None):
        patch_size=int(np.power(x.shape[0]-1, 1/2))
        with autocast():
            x_cls =x[0,...]
            # x_cls2=self.conv_clsB(self.dropout(self.conv_clsA(x_cls))).squeeze(-1)
            x_patch=x[1:,...].permute(1,2,0)
            x_patch = x_patch.view(x_patch.shape[0], x_patch.shape[1], patch_size, patch_size)

            x_patch=self.conv_B(self.conv_B_1(self.dropout(self.conv_A_2(self.conv_A(x_patch)))))
            x_patch=x_patch.view(x_patch.shape[0], x_patch.shape[1], -1).permute(2,0,1)
            x_patch=x_patch+x[1:,...]
            x = torch.cat((x_cls.unsqueeze(0), x_patch), dim=0)
            # x = torch.cat((x_cls.unsqueeze(0), x_patch), dim=0) + x
            # x[1:,...]=x_patch+x[1:,...]
        return x
...
def forward_vit(self, x: torch.Tensor):
    x = x + self.attn(self.adapter_attn(self.ln1(x)))
...
def set_MMAdapter(model, method, dim=8, s=1, set_forward=True,t=10,gradient_checkpointing=False):
    if method == 'block':
        ...
        for _ in model.children():
            if type(_) ==  phe_llm.model.TransformerBlock:
        ...
        for _ in model.children():
            if type(_) == phe_llm.model.TransformerBlock:
                _.adapter_attn = RepAdapter_Router(_.dim,hidden_dim=dim,scale=s,t=t)
                # _.adapter_attn = RepAdapter(_.dim, hidden_dim=dim, scale=s) #FY0829
                _.s = s
                _.t=t
                _.gradient_checkpointing = gradient_checkpointing
