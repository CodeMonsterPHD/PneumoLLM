...
class vitVisionTransformer(BaseBackbone):
    """Vision Transformer.
    ...
    def __init__(self,
                 ...
                 pre_norm=False,
                 init_cfg=None):
        super(vitVisionTransformer, self).__init__(init_cfg)
    ...
    def init_weights(self):
        super(vitVisionTransformer, self).init_weights()
    ...
    def forward(self, x, multiscale=4):
        B = x.shape[0]
        ...
        for i, layer in enumerate(self.layers):
            ...
            if (i+1) % multiscale==0:
                ret_feats.append(x[:,:1,:])  #[16, 257, 1024] -> append [16, 1, 1024]
...
if __name__ == '__main__':
    ...
    pretrained_cfg = '/disk1/wjr/.cache/vit/vit-large-p16_in21k-pre-3rdparty_ft-64xb64_in1k-384_20210928-b20ba619.pth'

    backbone=vitVisionTransformer(
        arch='l',
        img_size=224,
        patch_size=16,
        drop_rate=0.1,
    ...
    ck=torch.load(pretrained_cfg)
    model_dict = backbone.state_dict()
    # 关键在于下面这句，从model_dict中读取key、value时，用if筛选掉不需要的网络层
    pretrained_dict = {key.split('backbone.')[-1]: value for key, value in ck.items() if
                       (key.split('backbone.')[-1] in model_dict)}
    backbone.load_state_dict(pretrained_dict, strict=True)
    # 将参数加载到模型中
    aa=1
    bb=torch.rand(1,3,224,224)
    cc=1
    dd=net.backbone(bb)
    print(dd.shape)
